      ...
      if (profileModal && profileContent) {
        profileContent.innerHTML = `
          ...
          <p><strong>Permissions:</strong> ${
            Array.isArray(localUser.permissions)
              ? localUser.permissions.join(", ")
              : "None"
          }</p>
          <p><strong>Logged in:</strong> ${
            localUser.ts
              ? new Date(localUser.ts).toLocaleString()
              : "Unknown"
          }</p>
        `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const creatorOption = document.getElementById("creatorOption");
    ...
    taskForm.addEventListener(
      ...
      async (e) => {
        ...
        const creatorUsername =
          creatorOption?.value || user.email || "Unknown";
        const assigneeName =
        ...
        try {
          const snap = await getDocs(
            query(
              collection(db, "users"),
              where("username", "==", assigneeName)
            )
          );
          if (!snap.empty) {
            const data = snap.docs[0].data();
            assigneeId = data.uid || snap.docs[0].id;
          }
        } catch (err) {
        ...
        try {
          ...
          showToast(
            "⚠️ Error saving task: " + (err.message || "Unknown error")
          );
        } finally {
